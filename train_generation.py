...
def load_train_seqs(train_seqs_file):
	...
	train_seqs = (seqs[0].values.tolist() for seqs in pandas.read_csv(train_seqs_file, encoding='utf-8', header=None, chunksize=100000))
	return train_seqs

def create_model(model_type, save_filepath, min_freq, generalize_ents, batch_size, n_timesteps, n_hidden_layers, n_embedding_nodes, n_hidden_nodes, n_feature_nodes=None):
	...
	if model_type == 'feature_rnn':
		assert(n_feature_nodes)
		classifier = FeatureRNNLM(verbose=1, batch_size=batch_size, n_timesteps=n_timesteps, n_hidden_layers=n_hidden_layers, n_embedding_nodes=n_embedding_nodes,
								n_hidden_nodes=n_hidden_nodes, n_feature_nodes=n_feature_nodes, filepath=save_filepath)

	elif model_type == 'rnn':
		classifier = RNNLM(verbose=1, batch_size=batch_size, n_timesteps=n_timesteps, n_hidden_layers=n_hidden_layers, n_embedding_nodes=n_embedding_nodes,
	    					n_hidden_nodes=n_hidden_nodes, filepath=save_filepath)
...
def train_model(train_seqs_file, model, n_epochs):
	...
	for epoch in range(n_epochs):
	    ...
	    for seqs in load_train_seqs(train_seqs_file):
			...
			samp_seqs = [segment(seq) for seq in random.sample(seqs, 25)]
			context_seqs = [" ".join(seq[:-1]) for seq in samp_seqs]
			gold_seqs = [seq[-1] for seq in samp_seqs]
			gen_seqs = model.predict(seqs=context_seqs, n_best=1, mode='random', batch_size=25,
...
if __name__ == '__main__':
	...
	parser.add_argument("--n_feature_nodes", "-feat", help="For Feature RNN, specify number of nodes in feature hidden layer. Default is 100.", required=False, type=int, default=100)
	...
	args = parser.parse_args()
 	...
 	model = create_model(model_type=args.model_type, save_filepath=args.save_filepath, min_freq=args.min_freq, generalize_ents=args.generalize_ents, batch_size=args.batch_size, n_timesteps=args.n_timesteps, n_hidden_layers=args.n_hidden_layers, 
 						n_embedding_nodes=args.n_embedding_nodes, n_hidden_nodes=args.n_hidden_nodes, n_feature_nodes=args.n_feature_nodes)
